...
class StOMPAlgorithm : public Algorithm {
	...
	public:
		...
		void RunAlgorithm(double* vSignal, double* rSignal, double* mDictionary, double* fullDictionary) override {
			...
			while(check>=tolerance && iteration<1000){
				...
				for(int i=0;i<nAtoms;i++)
					std::cout << M[i] << ' ';
				std::cout << std::endl;*/
				double random = ((double) std::rand() / (RAND_MAX)) + 2;
				double lambda = random * sqrt(real_inner_product(residue,residue,szSignal)) / sqrt(szSignal);
				for(int i=0;i<nAtoms;i++){
					if(M[i]>=lambda){
						if(indices[i] == false)
							StOmp_sz++;
				...
				/*for(int i=0;i<StOmp_sz;i++){
						for(int j=0;j<szSignal;j++){
						std::cout << tmp_darr[i*szSignal+j] << ' ';
					}
					std::cout << std::endl;
				}*/
